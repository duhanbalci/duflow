...
- Export şeması değişirse `export.rs::SCHEMA_VERSION` ve `ui/src/graph.ts` tipleri birlikte (şu an 3: `in`, `outcomes`, `every`, `perms`, `entries`).
...
- Yerel var: guard'daki noktasız, tanımsız isim (`Graph::is_local_var`); lint aynı kapsamda (`model::local_group` = son noktaya kadarki önek) `sets` arar.
- Kenar prop'ları: `case` (dal), `seq` (sıra), `when` (guard); üçü de `ambiguous_transition`'ı susturur. Hedefsiz `-> outcome=` → `Outcome{arrow:true}`. `entry=` attr'lı node root gibi erişilebilirlik başlangıcı (`model::is_entry`), `too_many_roots` yalnız `every`'siz root'ları sayar.
- `duflow edit`: silme op'ları (`rm-edge`, `rm-child`) ekleme op'larından önce (`main.rs::edit_ops`); `rm --force` check/perm tanımındaki `-> x`/`fail_to` ve `reads` kelimesini de temizler.
